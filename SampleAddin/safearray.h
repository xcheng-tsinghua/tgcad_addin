...
public:
	...
	operator T* ()			{return m_arrayData;}
	operator VARIANT ()		{UnaccessData(); return *m_input;}
	T & operator[](int i)	{return m_arrayData[i];}
private:
	void UnaccessData(){
		if(m_access){
...
typedef SafeArray<BYTE,    VT_UI1>  SafeBYTEArray;
